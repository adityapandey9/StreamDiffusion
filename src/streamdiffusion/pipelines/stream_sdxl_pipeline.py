...
class StreamSDXLPipeline(StreamDiffusion):
    def __init__(
        self,
        ...
        vae_id : Optional[str], optional
            ...
            ("madebyollin/taesdxl") will be used.
    ...
    def _load_model(
        ...
        try:  # Load from local directory
            pipe: DiffusionPipeline = StableDiffusionXLPipeline.from_pretrained(
                model_id_or_path,
            ).to(device=self.device, dtype=self.dtype)

        except ValueError:  # Load from huggingface
            pipe: DiffusionPipeline = StableDiffusionXLPipeline.from_single_file(
                model_id_or_path,
            ).to(device=self.device, dtype=self.dtype)
        ...
        if not self.sd_turbo:
            if use_lcm_lora:
                if lcm_lora_id is not None:
                    ...
                    self.load_lcm_lora(pretrained_model_name_or_path_or_dict="latent-consistency/lcm-lora-sdxl")
                self.fuse_lora()
